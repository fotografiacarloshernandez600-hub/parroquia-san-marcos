...
export async function saveRecordAction(table, formData) {
  ...
  for (const [name, def] of Object.entries(conf.campos)) {
    if (def.tipo === 'image') {
      ...
      if (file && typeof file === 'object' && file.size > 0) {
        ...
        if (errUpload) {
          errorMsg = `No se pudo subir la imagen de "${def.label}": ${errUpload.message || 'error desconocido'}`;
          break;
        }
  ...
  let dbError = null;
  if (id > 0) {
    const { error } = await db.from(table).update(datos).eq('id', id);
    dbError = error;
  } else {
  ...
  if (dbError) {
    const destino = id > 0 ? `/admin/manage/${table}/${id}` : `/admin/manage/${table}/new`;
    const msg = `No se pudo guardar: ${dbError.message || 'error desconocido'}`;
    redirect(`${destino}?error=${encodeURIComponent(msg)}`);
  }
